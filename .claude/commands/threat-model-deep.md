...
2. **Choose a filename.**
   - No target argument: `threat-model-deep-YYYY-MM-DD.docx`
   - Named target: `threat-model-deep-<sanitized-target>-YYYY-MM-DD.docx` (lowercase, spaces → hyphens, strip slashes)
   - Use today's date.
